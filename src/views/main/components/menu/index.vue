...
<template>
  <div class="flex h-[80dvh] flex-col pb-[max(16px,env(safe-area-inset-bottom))]">
    <header
      class="flex items-center justify-between border-b border-zinc-100 px-[20px] py-[16px] dark:border-zinc-800"
    >
      <div>
        <p class="mb-[2px] text-xs font-medium tracking-[0.12em] text-zinc-400 dark:text-zinc-500">
          CATEGORY
        </p>
        <h2 id="category-menu-title" class="text-xl font-bold text-zinc-950 dark:text-zinc-50">
          所有分类
        </h2>
      </div>
      ...
      <button
        type="button"
        class="grid size-[38px] place-items-center rounded-full bg-zinc-100 text-zinc-700 transition-colors active:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:active:bg-zinc-700"
        aria-label="关闭分类菜单"
        @click="emit('close')"
      >
